...
#### 4.3.9 候选报文的格式与转发：发 → 路由 → 收 三跳拆解

> 对应代码：客户端 `signaling.dart` `onIceCandidate` 717–737 行、`onMessage` 的 `case 'candidate'` 438–458 行；服务端 `signaler.go` `case Candidate` 260–283 行

本节回答三个具体问题：**候选报文长什么样？服务端怎么解析它？收到后又怎么还原成候选？**

##### 发的三个字段：候选本体 + 归属信息

`onIceCandidate` 把候选拆成三个字段打包发出：
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `candidate` | String | 候选**本体**，就是那一行文本（格式见下） |
| `sdpMid` | String? | 候选属于 SDP 里哪条媒体流（对应 `a=mid:` 的值，如 `"0"` / `"video"`） |
| `sdpMLineIndex` | int? | 同上，只是改用 `m=` 段的**下标**表示（第 0 个 / 第 1 个） |

**为什么必须拆成三个？** 因为候选本体那一行**不包含「属于哪条流」的信息**。一个 PC 可以有多个 `m=` 段（audio / video / data），对方必须知道把它挂到哪条流上，否则会把 video 的候选加到 audio 上。`sdpMid` 与 `sdpMLineIndex` 是等价的两种索引方式。

> 注意：这一跳**并没有解析**——`candidate.candidate` 已经是原生层给出的完整字符串，Dart 只是「拆包」塞进 JSON。真正的解析在 libwebrtc 原生层。

##### candidate 字符串的格式（RFC 5245 §15.1）

空格分隔，**前 8 个字段必填，后 5 个可选**：

```text
candidate:<foundation> <component-id> <transport> <priority> <conn-addr> <port> typ <cand-type> [raddr <rel-addr>] [rport <rel-port>] [extensions]
```
...
candidate:1 1 udp 2130706431 192.168.1.10   54321 typ host
candidate:2 1 udp 1694498815 203.0.113.7    54322 typ srflx raddr 192.168.1.10 rport 54321
candidate:3 1 udp 16777215   47.251.242.213 49152 typ relay raddr 0.0.0.0 rport 0 generation 0 ufrag Xy7z network-cost 999
```
...
| # | 字段 | 示例 | 含义 |
| --- | --- | --- | --- |
| 1 | foundation | `1` | 同源候选共享同一个值；用于去重与剪枝（同 foundation 不必重复做连通性检查） |
| 2 | component-id | `1` | `1`=RTP，`2`=RTCP（unified-plan 下基本恒为 1） |
| 3 | transport | `udp` | `udp` 或 `tcp` |
| 4 | priority | `2130706431` | 优先级，**越大越优先**；由类型算出（见下） |
| 5 | conn-addr | `192.168.1.10` | 候选 IP（本机 IP / 公网映射 IP / 中继 IP） |
| 6 | port | `54321` | 候选端口 |
| 7 | `typ` | `typ` | 固定字面量分隔符 |
| 8 | cand-type | `host` | `host` / `srflx` / `prflx` / `relay` |
| 9–12 | `raddr` + rel-addr + `rport` + rel-port | `raddr 192.168.1.10 rport 54321` | 仅 srflx / prflx / relay 有：该候选的「基地址」（relay 场景常为 `0.0.0.0` / `0`） |
| 13 | extensions | `generation 0` | 扩展属性；Chrome 系会带 `generation` / `ufrag` / `network-cost` |

**priority 的计算公式**（RFC 8445 §5.1.2.1）：
...
所以 host 的 priority 恒最大（≈2.13×10⁹），relay 恒最小——「relay 只做兜底」在候选字符串里就体现为这个数字。

##### 服务端：几乎不解析，只认 `to`

```go
var negotiation Negotiation          // 只有 From / To / SessionID 三个字段
err := json.Unmarshal(body, &negotiation)
to := negotiation.To
peer, ok := s.peers[to]
if !ok { /* 回 error：Peer not found */ }
s.Send(peer.conn, request)           // request.Data 是原始 JSON，原样透传
```

两个关键点：

1. `Negotiation` 结构体只有 `From` / `To` / `SessionID`，**根本没有 `candidate` 字段**——服务端不关心候选内容；
2. `request.Data` 是 `*json.RawMessage`（原始 JSON），所以 `candidate` 部分被**原封不动**转发。

此外 `Candidate` 与 `Offer` / `Answer` 走的是**同一个 case**（`case Offer: fallthrough case Answer: fallthrough case Candidate:`），三者共用「取出 `to` → 查 `peers` → 原样转发」这一套逻辑。

这就是**无状态中继**：服务端只做「地址簿 + 转发」，不理解也不校验 SDP/候选语义。好处是简单、易横向扩展；代价是无法做候选过滤、改写或审计。

> 想验证：`signaler.go` 的 `logger.Infof("On message %v", string(message))` 会把原始报文打进日志，能看到完整 JSON。
...
把 3 个字段重新拼成 `RTCIceCandidate(candidate, sdpMid, sdpMLineIndex)`（顺序刚好是构造器形参顺序），再交给 `pc.addCandidate()`。之后是乱序缓冲的三个分支：

| 情况 | 处理 | 原因 |
| --- | --- | --- |
| `session != null && session.pc != null` | 直接 `addCandidate` | 正常路径，PC 已建好 |
| `session != null && session.pc == null` | 存入 `remoteCandidates` | PC 还没创建 |
| `session == null` | 新建 Session 并把候选存进去 | **候选比 offer 先到**（极端乱序） |

等 `setRemoteDescription` 完成后统一 flush（`signaling.dart` 393–398、414–419 行，详见 4.3.7）。

##### 这一跳在整体流程中的位置

**一句话：SDP 决定「说什么话」，candidate 决定「走哪条路」——没有候选交换，SDP 协商得再好也连不通。**

| 步骤 | 消息 | 作用 |
| --- | --- | --- |
| 1 | `offer` / `answer`（SDP） | 协商编解码/媒体方向，并交换 ICE 凭据 `a=ice-ufrag` / `a=ice-pwd` / `a=fingerprint` |
| 2 | **`candidate`** | 双方互告「我这边可能能通的地址」——host / srflx / relay |
| 3 | （原生层）连通性检查 | 在「A 的候选 × B 的候选」组成的候选对上互发 STUN Binding |
| 4 | （原生层）选路 | 选「优先级最高且连通」的那一对，媒体走它 |

四个关键点：

- **候选是双向的**：A 发给 B、B 也发给 A；只有双方候选**两两配对**才可能连通。单边全是内网 host 就配不出路径。
- **`ice-ufrag` / `ice-pwd` 是第 3 步的凭据**，它们在 **SDP 里**交换，不在 candidate 消息里。连通性检查的 STUN Binding 用 `USERNAME = 本地ufrag:远端ufrag` + `MESSAGE-INTEGRITY = HMAC-SHA1(ice-pwd, 报文)` 互相认证，防止伪造候选。**这正是收端必须缓冲乱序候选的根本原因**：凭据没到（SDP 还没处理），候选到了也用不了。
- **`sdpMid` / `sdpMLineIndex` 决定候选挂到哪条流**，多 `m=` 段时挂错就整条流连不上。
- **选中的类型决定成本**：host / srflx = 直连（免费）；relay = 走 19302 服务器中继（费带宽、增延迟）。

> 一句话总结：**`onIceCandidate` 把原生层的候选「拆成 3 个字段」；服务端只解析 `to` 再原样转发（无状态中继）；接收端把 3 个字段「重组」成 `RTCIceCandidate` 并用缓冲解决乱序。candidate 消息的作用是把双方的可用路径互通有无，让原生层的连通性检查有的放矢——这是 P2P 能打通的最后一块拼图。**
...
- [ ] 路由只认 `to/from/session_id`：`Offer/Answer/Candidate` 原样转发（Candidate 的解析/透传已见 4.3.9，本节补 Offer/Answer 与整体设计）
...
- [x] 4.3 ICE 与候选交换（候选三兄弟来源、STUN/TURN 探测报文、trickle 与乱序缓冲、sleep 1s 原因、candidate 报文格式与三跳转发）
- [ ] 4.1 信令定位与消息分类
...
- 客户端候选回调：`rephone-security/lib/services/signaling.dart` → `onIceCandidate()`（拆包发送，见 4.3.9）
