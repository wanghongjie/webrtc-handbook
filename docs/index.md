...
3. **深入底层**：想弄清"画面到底怎么从 native 跑到屏幕"、"数据怎么被采出来"——直接翻 `08` / `09` 两章（可独立查阅，含大量源码行号）。
4. **记录问题**：遇到 bug / 疑问，按 `07-问题与坑位记录.md` 的模板追加一条，并在本文「完善进度表」登记。
5. **配套资料（仓库外私有，本书不含）**：`04`、`06` 章提到的 `docs/监控端WebRTC信令连通逻辑.md`（连通全链路时序）属于 Rephone Security 私有项目文档，未随本书发布；仅在你本地工作区可读。
...
│   ├── 07-问题与坑位记录.md    ← 【骨架+3条】踩坑仓库，随遇随记
│   ├── 08-媒体流与渲染链路.md  ← 【已填充】穿透 Native：stream/textureId/Surface/音视频消费/释放/截图链路
│   └── 09-采集端与数据格式约定.md ← 【已填充】采集→编码→RTP/SRTP、格式分层、两端约定清单
...
- **进阶（可随时跳读）**：`08` / `09` 是把 `03` 的"媒体基础"下沉到 Flutter/原生实现的一手记录，排查黑屏、花屏、无声、延迟时最有用。
...
| 03 音视频与媒体基础 | ⚪ 仅骨架 | 2026-09-09 | 见文内「待填充清单」（可吸收 08/09 的结论回填） |
| 04 信令与连接建立 | 🟡 骨架 + 4.0/4.3 已填充 | 2026-09-11 | TURN(4.0.3)、ICE 全流程(4.3) 已完成；待补 Offer-Answer(4.2)/DTLS-SRTP(4.4)/服务端剖析(4.5) |
| 05 状态机与生命周期 | ⚪ 仅骨架 | 2026-09-09 | 见文内「待填充清单」 |
| 06 项目代码地图 | 🟡 骨架+初步索引 | 2026-09-09 | 见文内「待填充清单」 |
| 07 问题与坑位记录 | 🟡 骨架+3 条 | 2026-09-23 | 随项目填充 |
| 08 媒体流与渲染链路 | 🟢 已填充 | 2026-09-23 | 8.10 已补「截图链路 + 平台差异」；可补自建纹理的实践样例 |
| 09 采集端与数据格式约定 | 🟢 已填充 | 2026-09-21 | 可补 simulcast/BWE 实测数据、自研 VideoSink 完整示例 |
...
6. **第三方库路径简写**：`flutter_webrtc-1.6.0/` 指 `~/.pub-cache/hosted/pub.flutter-io.cn/flutter_webrtc-1.6.0/`（08/09 章大量引用，版本升级后需重新核对行号）。
